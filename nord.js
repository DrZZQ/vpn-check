...
serverList.forEach(function(item, i) {
...
if (s.pingAmount != 'all' && typeof s.pingAmount != 'string') {
  if (s.pingAmount > ip.length) {
    console.error('You set wrong')
    process.exit()
  } else {
    ...
    try {
      ...
      if (i == ip.length - 1) {
        bar.update(ip.length, {
          server: `Collect the folowing data, (~${s.pingTimeout} sec)`
        })
      }
      // }
      ...
      passServers++
    } catch (e) // console.log(serverList[i] + chalk.red(' no ping'))
